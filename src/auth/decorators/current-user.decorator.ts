...
export const CurrentUser = createParamDecorator((roles: ValidRoles[] = [], context: ExecutionContext) => {
    ...
    if (!user) {
        throw new InternalServerErrorException('No user inside the request - make sure we used the AuthGuard');
    }
    ...
    throw new ForbiddenException(`User needs a valid role [${roles.join(',')}]`);
});